...
import { X, Search, Play, Pause, Check, Music, ChevronRight, Loader2, ExternalLink, RefreshCw } from 'lucide-react';
import { MUSIC_CATEGORIES } from '../services/musicLibrary.js';
import { fetchJamendoTracks, searchJamendoTracks, getJamendoKey, saveJamendoKey } from '../services/jamendo.js';
...
function SetupScreen({ onSaved }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 text-center gap-5">
      <div className="w-16 h-16 rounded-2xl bg-brand-500/20 border border-brand-500/30 flex items-center justify-center text-3xl">🎵</div>
      <div>
        <h3 className="text-lg font-bold mb-1">Connect Music Library</h3>
        <p className="text-sm text-white/50 leading-relaxed">
          Get a <strong className="text-white">free</strong> Jamendo API key to browse 600,000+ licensed tracks — just like Instagram's music library.
        </p>
      </div>
      <a href="https://devportal.jamendo.com" target="_blank" rel="noopener noreferrer"
        className="flex items-center gap-2 text-brand-400 text-sm font-medium hover:text-brand-300 transition-colors">
        <ExternalLink className="w-4 h-4" />
        Get free key at devportal.jamendo.com
      </a>
      <div className="w-full space-y-2">
        ...
        />
        <button
          onClick={() => { if (key.trim()) { saveJamendoKey(key); onSaved(); } }}
          disabled={!key.trim()}
          className="btn-primary w-full py-3 disabled:opacity-40"
        >
          Connect & Browse Music
        </button>
      </div>
      <p className="text-xs text-white/30">Free plan · No credit card · Instant access</p>
    </div>
  );
}

export default function MusicPicker({ selectedTrack, onSelect, onClose }) {
  const [hasKey,      setHasKey]      = useState(!!getJamendoKey());
  ...
  const loadTracks = useCallback(async (cat, q) => {
    ...
    try {
      const results = q.trim()
        ? await searchJamendoTracks(q)
        : await fetchJamendoTracks(cat);
      setTracks(results);
    } catch (e) {
      if (e.message === 'NO_KEY') { setHasKey(false); }
      else setError('Could not load tracks. Check your API key.');
  ...
  useEffect(() => {
    ...
    queryTimeout.current = setTimeout(() => loadTracks(category, query), query ? 400 : 0);
    return () => clearTimeout(queryTimeout.current);
  }, [hasKey, category, query, loadTracks]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: '#0a0a0f' }}>
      ...
      {!hasKey ? (
        <SetupScreen onSaved={() => setHasKey(true)} />
      ) : (
        <>
          {/* Search */}
          <div className="px-4 py-3 flex-shrink-0">
            <div className="flex items-center gap-2 glass rounded-xl px-3 py-2.5">
              ...
              {query && (
                <button onClick={() => setQuery('')} className="text-white/30 hover:text-white/60"><X className="w-3.5 h-3.5" /></button>
              )}
            </div>
          </div>
