...
export default function VideoPreview({ script, currentScene, onSceneChange, voiceLang = 'en-US' }) {
  ...
  // Load Pexels videos when script changes
  useEffect(() => {
    ...
    if (!key) return;
    preloadSceneVideos(script.scenes, key).then(els => {
      videoEls.current = els;
      setVideosReady(true);
    });
  }, [script]);

  // Pause non-active videos, prep active one
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const draw = (timestamp) => {
      ...
      if (scene) {
        renderFrame(ctx, scene, script, sceneIndexRef.current, totalScenes, p, timestamp, bgVideo);
        if (isSpeaking) {
  ...
  }, [scene, isPlaying, isSpeaking, script, totalScenes, onSceneChange, stopSpeech, videosReady]);
  ...
  const videoCount = Object.keys(videoEls.current).length;
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="relative rounded-2xl overflow-hidden bg-dark-900 shadow-2xl shadow-black/50" style={{ aspectRatio: '16/9' }}>
        ...
        {script && pexelsKey && !videosReady && (
          <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/50 text-white/60 text-xs">
            <Film className="w-3 h-3 animate-pulse" /> Loading videos...
          </div>
        )}
        {videosReady && videoCount > 0 && (
          <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/50 text-green-400 text-xs">
            <Film className="w-3 h-3" /> {videoCount} videos loaded
          </div>
        )}
      </div>
      ...
      {script && !pexelsKey && (
        <p className="text-xs text-white/30 text-center">
          Add a <span className="text-brand-400">Pexels API key</span> in Settings → Stock Video for real video backgrounds
        </p>
      )}
    </div>
  );
