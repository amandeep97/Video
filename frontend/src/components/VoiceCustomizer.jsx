...
import { X, Play, Loader2, Check, Mic, Globe, Sparkles, ExternalLink } from 'lucide-react';
import {
  VOICE_LANGUAGES, ELEVENLABS_VOICES,
  getVoiceSettings, saveVoiceSettings,
  getElevenLabsSettings, saveElevenLabsSettings,
  fetchHuggingFaceTTS, speakBrowser,
  getAllVoices, getAvailableVoices,
...
const SAMPLE_TEXTS = {
  ...
  'en-US': 'Hello! This is a test voice. I will narrate your video with this voice.',
  'en-GB': 'Hello! This is a test voice. I will narrate your video with this voice.',
  default: 'Hello! This is a test of the selected voice for your AI video.',
};

export default function VoiceCustomizer({ onClose, voiceLang = 'en-US' }) {
  ...
  useEffect(() => {
    getAllVoices().then(v => setVoices(v));
  }, []);
  ...
  const sampleText = SAMPLE_TEXTS[voiceLang] || SAMPLE_TEXTS.default;
  ...
  const handleTest = async () => {
    ...
    try {
      if (provider === 'hf') {
        const blob = await fetchHuggingFaceTTS(sampleText, voiceLang);
        const url  = URL.createObjectURL(blob);
        const aud  = new Audio(url);
        aud.onended = () => { setTesting(false); URL.revokeObjectURL(url); };
        aud.onerror = () => { setTesting(false); setTestError('Playback failed'); URL.revokeObjectURL(url); };
        audioRef.current = aud;
        await aud.play();
        return;
      }
      if (provider === 'browser') {
        const { apiKey: elKey, voiceId } = elSettings;
        const utt = await speakBrowser(sampleText, voiceLang, () => setTesting(false));
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex gap-3 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
              ...
              <div className="text-xs text-white/70 leading-relaxed">
                <strong className="text-white">HuggingFace MMS</strong> — Meta's free AI TTS supporting
                Hindi, Punjabi, English, and 1000+ more languages. Works without a token, but adding a
                free token gives you more requests.
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs text-white/50">HuggingFace Token (optional)</label>
              ...
              <input type="text" value={hfToken} onChange={e => setHfToken(e.target.value)}
                placeholder="hf_... (optional, for higher limits)"
                className="input-field text-sm font-mono" />
              <p className="text-[10px] text-white/30 mt-1">Works without token · Token removes rate limits · 100% free</p>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {['hi-IN', 'pa-IN', 'en-US', 'es-ES', 'fr-FR', 'ar-SA'].map(l => {
                const lang = VOICE_LANGUAGES.find(v => v.id === l);
                return (
                  <div key={l} className="glass rounded-xl p-2 text-center">
                    <p className="text-base">{lang?.flag}</p>
                    <p className="text-[10px] text-white/60">{lang?.label}</p>
                    <p className="text-[9px] text-green-400">✓ AI voice</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="space-y-3">
            ...
            {filteredVoices.length === 0 ? (
              <div className="glass rounded-xl p-4 text-center">
                <p className="text-sm text-white/40">No voices found for this language on your device.</p>
                <p className="text-xs text-white/30 mt-1">Try English or check your device's language settings.</p>
              </div>
            ) : (
        ...
        {provider === 'elevenlabs' && (
          <div className="glass rounded-xl p-4 text-center space-y-2">
            <p className="text-sm text-white/60">ElevenLabs uses the key you set in <strong className="text-white">Settings → Voice</strong></p>
            <p className="text-xs text-white/40">Premium quality · 10,000 chars/month free · Realistic emotion</p>
          </div>
        )}

        {/* Speed & Pitch (browser + hf browser playback) */}
        {provider !== 'elevenlabs' && (
          <div className="space-y-3">
            <div>
        ...
        <div className="space-y-2">
          <button onClick={handleTest} disabled={testing}
            ...
            {testing
              ? <><Loader2 className="w-4 h-4 animate-spin" /> Testing voice…</>
              : <><Play className="w-4 h-4" /> Test Voice</>
            }
          </button>
          {testError && <p className="text-xs text-red-400 text-center">{testError}</p>}
          {provider === 'hf' && (
            <p className="text-[10px] text-white/30 text-center">First test may take ~20s while model loads on HuggingFace servers</p>
          )}
        </div>
      </div>
